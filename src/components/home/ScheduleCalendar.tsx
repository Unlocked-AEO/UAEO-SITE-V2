        ...
        <div className="flex gap-8">
          ...
          <div className="grow shrink basis-[0%] rounded-2xl bg-white border border-solid border-[#E6EBF1] overflow-clip">
            ...
            <div className="flex items-center justify-between py-4 px-6 border-b border-b-solid border-b-[#E6EBF1]">
              <h3 className="text-[#0A2540] font-sans font-semibold text-base/5 m-0">
                Select a time
              </h3>
              <div className="flex items-center gap-2">
                <span className="text-[#64748B] font-sans text-sm/4.5">
                  Week of Apr 7, 2025
                </span>
              ...
              {scheduleWeek.map((day, dayIdx) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center py-3 border-b border-b-solid border-b-[#E6EBF1] bg-[#FAFBFC]">
                    <span className="text-[#94A3B8] font-sans text-[11px]/3.5 uppercase tracking-[0.5px]">
                      {day.day}
                    </span>
                    <span className="text-[#0A2540] font-sans font-semibold text-sm/4.5">
                      {day.date}
                    </span>
                  </div>
                    ...
                    {day.slots.map((slot) => {
                      ...
                      return (
                        <button
                          ...
                          className={`py-2.5 rounded-lg text-center font-sans text-[13px]/4 border-none cursor-pointer transition-colors ${
                            isSelected
          ...
          <div className="w-[380px] shrink-0 flex flex-col rounded-2xl bg-white border border-solid border-[#E6EBF1] p-7">
            <h3 className="text-[#0A2540] font-sans font-semibold text-base/5 mb-1 m-0">
              Your details
            </h3>
            <p className="text-[#94A3B8] font-sans text-[13px]/4 mb-6 m-0">
              {selectedSlot
                  ...
                  <div key={rowIdx} className="flex gap-3">
                    {pair.map((field) => (
                      <div key={field.label} className="grow shrink basis-[0%] flex flex-col gap-1.5">
                        <label className="text-[#0A2540] font-sans font-semibold text-[12px]/3.5">
                        ...
                        {field.type === "select" ? (
                          <div className="h-10 flex items-center justify-between rounded-lg px-3 bg-white [border-width:1.5px] border-solid border-[#E2E8F0] shrink-0">
                            <span className="text-[#94A3B8] font-sans text-[13px]/4">{field.placeholder}</span>
                            <ChevronDownIcon />
                          </div>
                        ) : (
            ...
            >
              <span className={`font-sans font-bold text-[14px]/4.5 ${selectedSlot ? "text-[#0A2540]" : "text-[#94A3B8]"}`}>
                Confirm booking
              </span>
            </button>

            <p className="mt-3 text-[11px] text-center leading-[150%] text-[#94A3B8] font-sans m-0">
