...
function AnimatedStat({ stat }: { stat: typeof productStats[number] }) {
  const numMatch = stat.value.match(/([\d.]+)/);
  const num = numMatch ? parseFloat(numMatch[1]) : 0;
  const prefix = stat.value.match(/^[^\d]*/)?.[0] || "";
  const suffix = stat.value.match(/[^\d.]*$/)?.[0] || "";
  const [ref, animatedValue] = useScrollCountUp(Math.round(num), 1400);
  ...
  return (
    <section className="relative overflow-clip">
      ...
      <div className="relative flex flex-col items-center pt-24 pb-20 px-10 bg-white">
        ...
        {/* Badge */}
        <div className={`relative inline-flex items-center mb-6 rounded-[20px] py-1.25 px-3.5 bg-[#F0FDFA] border border-solid border-[#4ECDC459] transition-all duration-700 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
        ...
        {/* Subtext */}
        <p className={`relative text-[18px] leading-[165%] max-w-[560px] mb-10 text-center text-[#64748B] font-sans transition-all duration-700 delay-300 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          {productHero.subtext}
        </p>

        {/* CTAs */}
        <div className={`relative flex items-center gap-3.5 transition-all duration-700 delay-450 ${loaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          <button
