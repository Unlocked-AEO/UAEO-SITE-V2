...
Two layers: large ambient teal/purple gradient blobs (background) + 7-node logo network (foreground). The blobs are HTML divs centered behind the SVG network.

| Element | Animation | Trigger |
|---------|-----------|---------|
| 3 ambient gradient blobs (1100px, 750px, 650px) | GSAP continuous float with sine.inOut, different speeds/directions (8-10s cycles) | Continuous |
...
| Random connections | Flash bright (opacity 0.6, width 2.5px) every 1.8s, 1–2 at once | Continuous |
| All engine nodes | Attract toward mouse cursor on hover | Mouse move |
| Nearby connections | Brighten + thicken when mouse is within 130px | Mouse move |
...
**Note**: LogoBar ("Trusted by teams already winning on AI") was removed from the landing page — the engine logos are now visible in the hero network instead.
