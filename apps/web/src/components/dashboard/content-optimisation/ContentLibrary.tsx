...
import { listLibrary, deleteLibraryItem } from "@/lib/api/client";
...
interface ContentLibraryProps {
  ...
  /** Bumping this triggers a refetch — used after Approve+Save in the engine flow. */
  refreshKey?: number;
}
...
export function ContentLibrary({ onNewContent, onOpenItem, refreshKey = 0 }: ContentLibraryProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === "score-high") result = [...result].sort((a, b) => b.score - a.score);
    if (sort === "score-low") result = [...result].sort((a, b) => a.score - b.score);
    return result;
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* Grid */}
      {error && (
        <div className="rounded-xl py-4 px-5 bg-danger/5 border border-danger/30 text-danger text-[13px]/5">
          {error}
        </div>
      )}
      {loading ? (
        <div className="rounded-xl py-16 px-6 bg-white border border-border-light shadow-[0px_1px_4px_#0A25400F] text-center">
          <div className="text-slate-muted text-[13px]/5">Loading library…</div>
        </div>
      ) : filtered.length === 0 ? (
        <div className="rounded-xl py-16 px-6 bg-white border border-border-light shadow-[0px_1px_4px_#0A25400F] text-center">
          <div className="text-navy font-semibold text-base/6">
            {items.length === 0 ? "Your library is empty" : "No matches"}
          </div>
          <p className="mt-1 text-slate-muted text-[13px]/5">
            {items.length === 0
              ? "Approve and save a generated piece to see it here."
    ...
    <div className="lib-card flex flex-col rounded-xl bg-white border border-border-light shadow-[0px_1px_4px_#0A25400F] hover:border-teal/40 hover:shadow-[0px_4px_12px_#0A254014] transition-all">
      ...
      <div className="flex items-center justify-between px-5 py-3 border-t border-border-light">
        ...
        <div className="flex items-center gap-1">
          {/* Copy and Download icons require the source draft markdown,
              which the LibraryItem doesn't carry today. Backlog: add
              GET /library/:id/markdown then wire these. */}
          <IconButton
            label="Delete"
            onClick={(e) => {
              e.stopPropagation();
