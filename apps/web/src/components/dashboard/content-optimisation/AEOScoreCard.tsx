import { aeoSignals as defaultSignals, type AEOSignal } from "@/data/mock-content-optimisation";
...
export function AEOScoreCard({ signals = defaultSignals, totalScore }: AEOScoreCardProps) {
  const total = totalScore ?? signals.reduce((s, x) => s + x.score, 0);
  const max = signals.reduce((s, x) => s + x.maxScore, 0);
  const pct = max > 0 ? Math.round((total / max) * 100) : 0;
  const aboveThreshold = total >= 80;
  ...
  return (
    <div className="rounded-xl py-5 px-6 bg-white border border-border-light shadow-[0px_1px_4px_#0A25400F]">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] uppercase tracking-[0.4px] font-semibold ${
          aboveThreshold ? "bg-teal/10 text-teal" : "bg-warning/10 text-warning"
        }`}>
      ...
      <div className="flex items-end gap-2 mb-1">
        ...
        <span className="pb-2 text-slate-muted text-sm/5">/ {max}</span>
      </div>
